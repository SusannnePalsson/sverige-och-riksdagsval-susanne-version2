addMdToPage(`
  ...
  ## Medelinkomst grupperat på Kommunhuvudgrupp
`);

addMdToPage(`

  Diagrammet nedan visar medelinkomster för de olika Kommunhuvudgrupperna.  
...
tableFromData({
  data: kgi,
  columnNames: ["Huvudgrupp", "Kommungrupp", "Kommunnamn"]
});
*/
...
tableFromData({
  data: income,
  columnNames: ["kommun", "kon", "medelInkomst2018", "medelInkomst2019", "medelInkomst2020", "medelInkomst2021", "medelInkomst2022"]
});
*/
...
const joinedData = kgi
  .map(row => {
    ...
    if (incomeRow) {
      // Ensure all columns exist, set to null if missing
      return {
...
addMdToPage(`
  ...
  <select id="groupby-select">
    ...
    <option value="Kommungrupp">Kommungrupp</option>
  </select>
...
// Rita diagrammet baserat på val
function drawChart() {
  ...
  joinedData.forEach(row => {
    // Assign a default value if group key is missing or empty
  ...
  // Anpassa diagramrubrik och färger beroende på val
  let colors, title;
